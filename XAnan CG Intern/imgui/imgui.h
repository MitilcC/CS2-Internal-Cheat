...
namespace ImGui
{
    ...
    IMGUI_API bool          BeginChildPosEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags);
    IMGUI_API bool          BeginChildPos(const char* str_id, const ImVec2& size_arg, bool border = false, ImGuiWindowFlags extra_flags = 0);
    IMGUI_API bool          BeginChildPos(ImGuiID id, const ImVec2& size_arg, bool border = false, ImGuiWindowFlags extra_flags = 0);
    ...
    IMGUI_API bool          Tab(const char* ic0, const char* label, const char* hint, bool selected, const ImVec2& size = ImVec2(0, 0));   // button
    IMGUI_API bool          TABB(const char* label, bool selected, float SHIRINA = 123.f);

    IMGUI_API bool          SubTabEx(const char* ic0, bool selected, const ImVec2& size_arg, ImGuiButtonFlags flags);
    IMGUI_API bool          SubTab(const char* ic0, bool selected, const ImVec2& size_arg = ImVec2(0, 0));
    IMGUI_API bool          SmallButton(const char* label);                                 // button with FramePadding=(0,0) to easily embed within text
    IMGUI_API bool          InvisibleButton(const char* str_id, const ImVec2& size, ImGuiButtonFlags flags = 0); // flexible button behavior without the visuals, frequently useful to build custom behaviors using the public api (along with IsItemActive, IsItemHovered, etc.)
    IMGUI_API bool          ArrowButton(const char* str_id, ImGuiDir dir);                  // square button with an arrow shape
    IMGUI_API bool          Checkbox(const char* label, bool* v);
    IMGUI_API void          Colorbox(const char* label, bool* v, float col[4]);
    IMGUI_API void          binderbox(const char* label, const char* key_label, bool* v, int* key);
